...
class CoarseDetect(nn.Module):
    """
    创新点三：不确定性感知的主动采集头
    输出: [Saliency_Map, Uncertainty_Map, Crop_Params(tx, ty, s)]
    """
    def __init__(self, nc=1, ch=()):  # ch是输入通道数列表
        super().__init__()
        self.nc = nc  # number of classes (这里主要指前景/背景)
        self.nl = len(ch) # number of detection layers
        
        # 简单的卷积层预测
        # 1. Saliency Map (1 channel)
        # 2. Uncertainty Map (1 channel, variance)
        # 3. Crop Params (3 channels: tx, ty, scale)
        self.cv_saliency = nn.ModuleList(nn.Conv2d(x, 1, 1) for x in ch)
        self.cv_uncertainty = nn.ModuleList(nn.Conv2d(x, 1, 1) for x in ch)
        self.cv_crop = nn.ModuleList(nn.Conv2d(x, 3, 1) for x in ch)

    def forward(self, x):
        """
        x: list of features from backbone/neck
        """
        saliency_maps = []
        uncertainty_maps = []
        crop_params = []
        ...
        for i in range(self.nl):
            s = torch.sigmoid(self.cv_saliency[i](x[i]))     # 归一化到 0-1
            u = F.softplus(self.cv_uncertainty[i](x[i]))     # 方差必须 > 0
            c = torch.tanh(self.cv_crop[i](x[i]))            # tx, ty 在 -1 到 1 之间
            # Scale 单独处理，一般限制在 (0, 1]
            # 这里简单处理，取绝对值或sigmoid
            
            saliency_maps.append(s)
            uncertainty_maps.append(u)
            crop_params.append(c)

        return saliency_maps, uncertainty_maps, crop_params
    


# --- 创新点三：CoarseDetect (Start) ---
from .conv import Conv

class CoarseDetect(nn.Module):
    """
    Coarse Detection Head for Cascade Focusing.
    Structure:
        - Branch 1: Saliency Map (Output 1 channel, Sigmoid activation)
        - Branch 2: Uncertainty Map (Output 1 channel, Softplus activation)
    Output:
        [B, 2, H, W] per scale level.
    """
    dynamic = False  # force grid reconstruction
    export = False   # export mode
    shape = None
    ...
    def __init__(self, nc=1, ch=()): 
        """
        Args:
            nc (int): number of classes (not strictly used here, as we output Saliency)
            ch (tuple): input channels from Neck (e.g., [128, 256, 512])
        """
        super().__init__()
        self.nc = nc  
        self.nl = len(ch)  # number of detection layers (e.g., 3 for P3, P4, P5)
        self.no = 2  # Outputs: Channel 0 = Saliency, Channel 1 = Uncertainty

        # 定义两个卷积分支
        # cv2: 负责 Saliency
        # cv3: 负责 Uncertainty
        c2, c3 = max((16, ch[0] // 4, 64)), max(ch[0], 64)  # 内部通道数设定
    ...
    def forward(self, x):
        """
        Args:
            x (list): Features from FPN/Neck.
        Returns:
            list[Tensor]: Each tensor shape [B, 2, H, W] (Saliency, Uncertainty)
        """
        for i in range(self.nl):
            # Input x[i] shape: [B, C_in, H, W]
            
            # 1. 计算显著性 (Saliency) -> Range (0, 1)
            # a. 获取当前输入特征
            feature = x[i]
            
            # b. 检查维度，如果是 3D (C, H, W)，则增加 Batch 维度变为 (1, C, H, W)
            if feature.dim() == 3:
                feature = feature.unsqueeze(0)
            
            # c. 传入 cv2 计算 (此时保证是 4D)
            saliency = self.cv2[i](feature)
            saliency = torch.sigmoid(saliency)

            # 2. 计算不确定性 (Uncertainty / Variance) -> Range (0, +inf)
            # 使用 Softplus 保证方差为正数
            uncertainty = self.cv3[i](x[i])
            uncertainty = F.softplus(uncertainty)

            # 3. 拼接输出 -> Channel 0 is Saliency, Channel 1 is Uncertainty
            x[i] = torch.cat((saliency, uncertainty), 1)
            
        return x
# --- 创新点三：CoarseDetect (End) ---